...
import Compute from '../';
import type {Project, Rule, Subnetwork} from '../src';
...
describe('Compute', () => {
  ...
  function generateName(customPrefix: string) {
    const resourceId = uuid.v4().split('-')[0];
    return `${TESTS_PREFIX}-${TEST_ID}-${customPrefix}-${resourceId}`;
  }

  async function deleteAllTestObjects(opts: Record<string, unknown>) {
  ...
  async function deleteAllGcloudTestObjects(opts: Record<string, unknown>) {
    const objectTypes = [
      'VMs',
      'Addresses',
      'Autoscalers',
  ...
  async function callAndDeleteGcloudTestObject(
    type: string,
    opts: Record<string, unknown>
  ) {
    // @ts-expect-error
    let [objects]: [Metadata[]] = await compute[`get${type}`]({
  ...
  async function create<T>(object: ServiceObject, cfg?: T) {
    return awaitResult(object.create(cfg));
  }

  async function deleteRegionalRules(opts: Record<string, unknown>) {
    const [rules] = await region.getRules({
      filter: `name eq ${opts.name}.*`,
    });
    return Promise.all(
  ...
  async function deleteHttpsHealthChecks(opts: Record<string, unknown>) {
    const [healthChecks] = await compute.getHealthChecks({
      filter: `name eq ${opts.name}.*`,
      https: true,
    });
  ...
  async function deleteUrlMaps(opts: Record<string, unknown>) {
    const resp = await computeRequest({
      uri: '/global/urlMaps',
      qs: {
        filter: `name eq ${opts.name}.*`,
  ...
  async function deleteUrlMap(name: string) {
    const resp = await computeRequest({
      method: 'DELETE',
  ...
  async function deleteTargetProxies(opts: Record<string, unknown>) {
    const resp = await computeRequest({
  ...
  async function deleteTargetProxy(name: string) {
    const resp = await computeRequest({
      method: 'DELETE',
  ...
  async function deleteTargetInstances(opts: Record<string, unknown>) {
  ...
  async function createTargetInstance(name: string, instanceName: string) {
    const resp = await zoneRequest({
      method: 'POST',
      uri: '/targetInstances',
      json: {
  ...
  async function deleteTargetInstance(name: string) {
    const resp = await zoneRequest({
      method: 'DELETE',
  ...
  async function deleteInstanceTemplates(opts: Record<string, unknown>) {
    const resp = await computeRequest({
  ...
  async function createInstanceTemplate(name: string, networkName: string) {
    const resp = await computeRequest({
      method: 'POST',
      uri: '/global/instanceTemplates',
      json: {
  ...
  async function deleteInstanceTemplate(name: string) {
    const resp = await computeRequest({
      method: 'DELETE',
  ...
  async function deleteInstanceGroupManagers(opts: Record<string, unknown>) {
    const resp = await zoneRequest({
  ...
  async function createInstanceGroupManager(
    name: string,
  ...
  async function deleteInstanceGroupManager(name: string) {
    const resp = await zoneRequest({
      method: 'DELETE',
...
async function awaitResult(promise: Promise<any[]>) {
  const result = await promise;
  const operation = result[result.length - 2];
  await operation.promise();
}

function filterExpired(resources: Metadata[], opts: Record<string, unknown>) {
  resources = resources || [];
  if (opts.expiredOnly) {
...
function isExpired(resource: Metadata) {
  if (
    resource &&
