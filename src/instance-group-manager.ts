 ...
 */
export class InstanceGroupManager extends ServiceObject {
  zone: Zone;
  name: string;
    ...
    this.request(
      {
        ...
        json: {instances: arrify(vms).map(vm => vm.url)},
      },
      (err, resp) => {
        if (err) {
  ...
  getManagedInstances(
    ...
    >(options || {}, callback);
    this.request(
      {method: 'POST', uri: '/listManagedInstances', qs: opts},
      (err, resp) => {
        if (err) {
        ...
        const vms = arrify(resp.managedInstances as Metadata[]).map(vm => {
