...
import type {CreateFirewallOptions, Firewall} from './firewall';
import type {
...
import type {CreateSubnetworkOptions, Subnetwork} from './subnetwork';

export interface CreateNetworkOptions extends BaseOptions {
  gateway?: string;
  gatewayIPv4?: string;
  range?: string;
  IPv4Range?: string; // Deprecated
 ...
 */
export class Network extends ServiceObject {
  compute: Compute;
  ...
  createSubnetwork(
    ...
    if (is.string(region)) {
      region = this.compute.region(region as string);
    }
  ...
  getSubnetworks(
    ...
    >(options, callback);
    this.compute.getSubnetworks(
      Object.assign({}, opts, {filter: 'network eq .*' + this.formattedName}),
      cb
    );
  }
  /**
   ...
   *   });
   */
  getFirewallsStream(options?: GetResourcesOptions): ResourceStream<Firewall> {
    options = Object.assign({}, options, {
      filter: 'network eq .*' + this.formattedName,
    });
   ...
   */
  static formatName_(compute: Compute, name: string): string {
    return `projects/${compute.projectId}/global/networks/${name}`;
  }
}
 ...
 */
promisifyAll(Network, {exclude: ['firewall']});
