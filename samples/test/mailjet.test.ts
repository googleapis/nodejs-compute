...
import type {SmtpOptions} from 'nodemailer-smtp-transport';
...
describe('mailjet', () => {
  it('should send an email', () => {
    proxyquire('../mailjet', {
      nodemailer: {
        createTransport: (arg: TransportOptions) => {
          assert.strictEqual(arg, 'test');
          return {
            sendMail: (payload: SendMailOptions) => {
      ...
      'nodemailer-smtp-transport': (options: SmtpOptions) => {
