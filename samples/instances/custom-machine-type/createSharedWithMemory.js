...
/**
 * Creates a new VM instance with a custom type using shared CPUs.
 *
 ...
 */
function main(projectId, zone, instanceName, cpuSeries, memory) {
  ...
  class CustomMachineType {
    constructor(zone, cpuSeries, memory) {
      this.zone = zone;
      ...
      if (![this.E2Micro, this.E2Small, this.E2Medium].includes(cpuSeries)) {
      ...
      if (
        ...
        !this.typeLimt.allowedCores.includes(this.coreCount)
      ) {
        throw new Error(
    ...
    getMachineTypeURI() {
      ...
      if (this.memory > this.coreCount * this.typeLimit.maxMemPerCore) {
        return `zones/${this.zone}/machineTypes/${this.cpuSeries}-${this.coreCount}-${memory}-ext`;
      }

      return `zones/${zone}/machineTypes/${cpuSeries}-${this.coreCount}-${memory}`;
    }
  }

  async function createInstanceWithCustomSharedCore() {
  ...
  createInstanceWithCustomSharedCore();
